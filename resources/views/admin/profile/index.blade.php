...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ __('backend.Profile') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="{{ route('admin.dashboard') }}">{{ __('backend.Dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('backend.Profile') }}</div>
            </div>
        </div>
        <div class="section-body">
            <h2 class="section-title">{{ __('backend.Hi') }}, {{ $user->name }}!</h2>
            <p class="section-lead">{{ __('backend.Change information about yourself on this page') }}.</p>

            <div class="row">
                <div class="col-6">
                    <div class="card">
                        <div class="card-header">
                            <h4>{{ __('backend.Edit Profile') }}</h4>
                        </div>
                        <div class="card-body">
                            ...
                            <form method="POST" action="{{ route('admin.profile.update', auth()->guard('admin')->user()->id) }}" class="needs-validation" novalidate="" enctype="multipart/form-data">
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <button class="btn btn-primary">{{ __('backend.Save Changes') }}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
                ...
                <div class="col-6">
                    <div class="card">
                        <div class="card-header">
                            <h4>{{ __('backend.Update Password') }}</h4>
                        </div>
                        <div class="card-body">
                            ...
                            <form method="POST" action="{{ route('admin.update-password', auth()->guard('admin')->user()->id) }}" class="needs-validation" novalidate="" enctype="multipart/form-data">
                                ...
                                @method('PUT')
                                <div class="form-group row mb-4">
                                    <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __('backend.Current Password') }}</label>
                                    <div class="col-sm-12 col-md-7">
                                        <input type="password" class="form-control" name="current_password" required />
                                        @error('current_password')
                                            <div class="invalid-feedback" style="display: block">{{ $message }}</div>
                                        @enderror
                                        <div class="invalid-feedback">{{ __('backend.Please fill in the old password') }}</div>
                                    </div>
                                </div>
                                <div class="form-group row mb-4">
                                    <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">{{ __('backend.New Password') }}</label>
                                    <div class="col-sm-12 col-md-7">
                                ...
                                <div class="form-group row mb-4">
                                    <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3"></label>
                                    <div class="col-sm-12 col-md-7">
                                        <button class="btn btn-primary">{{ __('backend.Update') }}</button>
                                    </div>
                                </div>
                            </form>
                        </div>
